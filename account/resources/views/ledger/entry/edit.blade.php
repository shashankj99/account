...
@section('content-header')
    <div class="row mb-2">
        <div class="col-12 col-sm-4 col-md-2 mb-2">
            <a href="{{ route('entry.index') }}" class="btn btn-block btn-primary">
                <i class="fas fa-arrow-circle-left"></i> Go Back
            </a>
        </div>
        <div class="col-12 col-sm-8 col-md-10 mb-2">
            <div class="text-center">
                <h1 class="m-0 text-dark">Edit Ledger Entry</h1>
            </div>
        </div>
    </div>
@endsection
    ...
    <div class="row">
        <div class="col-12">
            <form action="{{ route('entry.update', $entry->id) }}" method="post">
                @csrf
                {{ method_field('PUT') }}
                @include('ledger.entry._form', ['buttonText' => 'Update', 'ledgerAccounts' => $ledgerAccounts, 'entry' => $entry])
            </form>
        </div>
    </div>
